...
//Open a terminal to interact with the user
try
{
    // Parse options
    ...
    ICodeFlowManager codeFlow = OAuth2CodeFlowManager.Create(Scope.Write, ConfigManager.Instance.ClientId, ConfigManager.Instance.ClientSecret, tokenStorage);
    ISheetManager sheet = GoogleApiConnector.Instance.CreateSheetManager(ConfigManager.Instance.SheetId, codeFlow);
...
{
    ConsoleReport.Instance.WriteInfo("Ummmm something happened");
}
finally
{
